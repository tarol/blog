...
### 延伸问题 1：那么为什么这个提案允许一个类同时存在私有成员 #x 和公有成员 x ？

1. 如果私有成员和公有成员冲突，会破坏其“封装性”。
...
> 译者按：所谓的“封装性”（encapsulation / hard private）是很重要的概念，[最底下](#encapsulation)会有说明。最简单的解释是，外部不能以任意方式获取私有成员的任何信息。假设，公有成员和私有成员冲突，而 `x` 是 `obj` 的私有成员，这时候外部存在 `obj.x`。如果公私冲突，这里将会报错，外部就嗅探到了 `obj` 存在 `x` 这个私有成员。这就违背了“封装性”。
...
> ```
>
> 以上的例子是一段正常的 Java 代码，它的逻辑很简单：声明类 A，A 存在一个公有方法，允许实例和另一个实例交换私有成员 p。
>
...
类方法经常操作不是实例的对象。当 `obj` 不是实例的时候，如果 `obj.x` 突然间不再指的是 `obj` 的公有字段 `x`，仅仅是因为在类的某个地方声明了私有成员 `x`，那就太奇怪了。
...
### <span id="Symbols">你怎么使用 `Symbols` 提供隐藏但不封装的属性？</span>
